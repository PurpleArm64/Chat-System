...
app.use(cors());
// Base64 আপলোডের জন্য লিমিট ২৫MB রাখা হয়েছে (MongoDB লিমিট ১৬MB, তাই ১০MB ফাইল সেফ)
...
let reelsCol, reelLikesCol, reelCommentsCol, musicCol, filesCol;
...
async function connectDB() {
    ...
    filesCol = db.collection('uploads'); // ✅ ফাইল স্টোরেজের জন্য কালেকশন
...
app.get('/api/ping', (req, res) => res.json({
    ok: true, t: Date.now(), db: dbReady, storage: 'mongodb', youtube: youtubeApiReady
}));
...
app.post('/api/upload', async (req, res) => {
    try {
        ...
        if (buf.length > 10 * 1024 * 1024) {
            return res.status(400).json({ error: 'File too large. Maximum 10MB allowed for MongoDB storage.' });
        }
        ...
        const url = `${req.protocol}://${req.get('host')}/api/files/${fileId}`;
        
        res.json({ url, publicId: fileId, provider: 'mongodb' });
    } catch (err) {
...
app.get('/api/files/:id', async (req, res) => {
    try {
        ...
        const file = await filesCol.findOne({ _id: new ObjectId(req.params.id) });
    ...
    try {
        ...
        if (avatar && avatar.startsWith('data:')) {
            ...
            if (match) {
                ...
                try {
                    const doc = { fileName: 'avatar_' + Date.now(), mime: mime, size: buf.length, data: buf, createdAt: Date.now() };
                    const result = await filesCol.insertOne(doc);
                    avatarUrl = `${req.protocol}://${req.get('host')}/api/files/${result.insertedId.toString()}`;
