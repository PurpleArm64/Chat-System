...
if (process.env.R2_ACCOUNT_ID && process.env.R2_ACCESS_KEY_ID && process.env.R2_SECRET_ACCESS_KEY && process.env.R2_BUCKET_NAME) {
    ...
    console.warn('⚠️ R2 NOT configured — media uploads WILL fail. Set R2_* env vars.');
}
...
function getYouTubeErrorFix(apiError) {
    ...
    const map = {
        'keyInvalid': 'Your API Key is invalid or deleted. Create a new API Key from Google Cloud Console.',
        'ipRefererBlocked': 'Your API Key has "HTTP referrers" or "IP Address" restriction enabled. This is blocking the Render server IP. Go to Google Console -> Credentials -> Your Key -> Application restrictions -> Set "None".',
        'forbidden': 'Your API Key has restrictions OR YouTube Data API v3 is not enabled. Go to Google Cloud Console -> APIs & Services -> Library -> "YouTube Data API v3" -> Enable it.',
        'quotaExceeded': 'Today daily limit (10,000 units) exceeded. It will reset at California midnight (Bangladesh time 12:30-1:00 PM).',
        'dailyLimitExceeded': 'Today daily limit exceeded. Try again tomorrow after quota reset.',
        'rateLimitExceeded': 'Sending requests too fast. Please wait a few seconds.',
        'accessNotConfigured': 'YouTube Data API v3 is not enabled for your Google Cloud Project. Go to Console -> Library and enable it.',
        'backendError': 'Google server error. Please try again in a moment.'
    };
    return map[reason] || `Unknown error (${reason}). Check your API Key and YouTube Data API v3 in Google Cloud Console.`;
}
...
app.get('/api/youtube/test', async (req, res) => {
    if (!youtubeApiReady) {
        return res.json({
            success: false,
            stage: 'config',
            error: 'YOUTUBE_API_KEY environment variable is missing or empty'
        });
    }
    ...
    try {
        ...
        if (!r.ok) {
            ...
            return res.json({
                ...
                errorReason: apiError.errors?.[0]?.reason,
                errorMessage: apiError.message,
                howToFix: getYouTubeErrorFix(apiError)
            });
        }
        ...
        res.json({
            ...
            message: 'YouTube API is working perfectly!'
        });
    } catch (err) {
        res.json({
            success: false,
...
async function uploadToR2(buf, mime, fileName, folder = 'uploads') {
    if (!r2Ready) throw new Error('R2 storage not configured. Set R2_* env vars.');
...
async function deleteFromR2(key) {
    if (!r2Ready || !key) return;
    try { await r2Client.send(new DeleteObjectCommand({ Bucket: R2_BUCKET, Key: key })); } catch (e) {}
}
...
app.post('/api/upload', async (req, res) => {
    try {
        ...
        if (!r2Ready) return res.status(500).json({ error: 'R2 storage not configured' });
...
app.get('/api/youtube/search', async (req, res) => {
    try {
        ...
        if (!r.ok) {
            const apiError = data.error || {};
            const reason = apiError.errors?.[0]?.reason || '';
            const fixMsg = getYouTubeErrorFix(apiError);
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        res.json({ user: {
            ...
            verified: false, hasPassword: true
        }});
...
connectDB().then(() => {
    ...
    console.log(`📦 Storage: MongoDB (Text/Users/Music) + R2 (Media: ${r2Ready ? 'OK' : 'NOT CONFIGURED'})`);
