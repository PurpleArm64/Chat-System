...
if (process.env.CLOUDINARY_CLOUD_NAME && process.env.CLOUDINARY_API_KEY && process.env.CLOUDINARY_API_SECRET) {
    try {
        ...
        console.error('Cloudinary init error:', e.message);
    }
} else {
    console.warn('⚠️ Cloudinary env vars not set');
}

let usersCol, requestsCol, friendshipsCol, messagesCol, notesCol, storiesCol;
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        res.json({ user: { id: result.insertedId.toString(), username, fullName, avatar: newUser.avatar, avatarType: newUser.avatarType, verified: false } });
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        res.json({ user: { id: user._id.toString(), username: user.username, fullName: user.fullName, avatar: user.avatar, avatarType: user.avatarType || 'letter', verified: !!user.verified } });
    } catch (err) { console.error(err); res.status(500).json({ error: 'Server error' }); }
});
...
app.post('/api/notes', async (req, res) => {
    try {
        ...
        await notesCol.deleteMany({ userId });
        if (text && text.trim()) {
            await notesCol.insertOne({ userId, text: text.trim().slice(0, 60), createdAt: Date.now() });
        }
        const friendIds = await getFriendIds(userId);
        friendIds.forEach(fid => {
            const sid = onlineUsers.get(fid);
            if (sid) io.to(sid).emit('note_updated', { userId });
        });
        res.json({ success: true });
    } catch (err) { res.status(500).json({ error: 'Server error' }); }
});
...
app.get('/api/notes', async (req, res) => {
    try {
        ...
        const notes = await notesCol.find({ userId: { $in: ids }, createdAt: { $gt: cutoff } }).toArray();
        const uids = notes.map(n => { try { return new ObjectId(n.userId); } catch (e) { return null; } }).filter(Boolean);
        ...
        const result = notes.map(n => {
            const u = map.get(n.userId);
            if (!u) return null;
            return {
                userId: n.userId, text: n.text, createdAt: n.createdAt,
                username: u.username, fullName: u.fullName,
                avatar: u.avatar, avatarType: u.avatarType || 'letter',
                verified: !!u.verified
            };
        }).filter(Boolean);
        result.sort((a, b) => (a.userId === userId ? -1 : b.userId === userId ? 1 : 0));
        res.json(result);
    } catch (err) { res.status(500).json({ error: 'Server error' }); }
});
