...
if (process.env.R2_ACCOUNT_ID && process.env.R2_ACCESS_KEY_ID && process.env.R2_SECRET_ACCESS_KEY && process.env.R2_BUCKET_NAME) {
    try {
        ...
        r2Ready = true;
        console.log('✅ R2 configured as PRIMARY storage');
    } catch (e) { console.error('❌ R2 init error:', e.message); }
} else {
    console.warn('⚠️ R2 NOT configured — uploads WILL fail. Set R2_* env vars.');
}
...
async function uploadToR2(buf, mime, fileName) {
    if (!r2Ready) throw new Error('R2 storage not configured. Admin: set R2_* env vars.');
...
app.post('/api/upload', async (req, res) => {
    try {
        ...
        if (buf.length > 25 * 1024 * 1024) return res.status(400).json({ error: 'File too large (max 25MB)' });
        if (!r2Ready) return res.status(500).json({ error: 'Storage not configured. Set R2_ACCOUNT_ID, R2_ACCESS_KEY_ID, R2_SECRET_ACCESS_KEY, R2_BUCKET_NAME, R2_PUBLIC_URL' });
        const result = await uploadToR2(buf, mime, fileName);
    ...
    try {
        ...
        if (avatar && avatar.startsWith('data:')) {
            ...
            if (match) {
                ...
                if (r2Ready) {
                    try {
                        const r = await uploadToR2(buf, mime, 'avatar_' + Date.now());
...
/* =========================================================
   ⚡ START SERVER INSTANTLY — NO TIMER, NO DELAY
   ========================================================= */
...
connectDB().then(() => {
    ...
    // Shorts warmup — 5 minutes after boot (background, non-blocking)
    setTimeout(() => {
        (async () => {
