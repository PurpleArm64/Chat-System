...
async function connectDB() {
    ...
    await starredCol.createIndex({ userId: 1, messageId: 1 }, { unique: true });
    await archivedCol.createIndex({ userId: 1, friendId: 1 }, { unique: true });
...
app.get('/api/ping', (req, res) => res.json({ ok: true, t: Date.now(), storage: { r2: r2Ready, cloudinary: cloudinaryReady }, youtube: youtubeApiReady }));
...
async function uploadToR2(buf, mime, fileName) {
    ...
    return { url: R2_PUBLIC_URL ? `${R2_PUBLIC_URL}/${key}` : `${process.env.R2_ACCOUNT_ID}.r2.cloudflarestorage.com/${R2_BUCKET}/${key}`, publicId: key, provider: 'r2' };
}
...
app.post('/api/upload', async (req, res) => {
    try {
        ...
        if (isAudio) {
            if (r2Ready) { try { result = await uploadToR2(buf, mime, fileName); } catch (e) { console.warn('R2 audio failed:', e.message); } }
            if (!result && cloudinaryReady) { try { result = await uploadToCloudinary(buf, mime); } catch (e) { console.warn('Cloudinary audio failed:', e.message); } }
        } else {
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        const newUser = { fullName, username, email, password, avatar: avatarUrl, avatarType, verified: false, lastSeen: Date.now(), createdAt: Date.now(), blockedUsers: [], blockedBy: [], forceLogoutAt: 0 };
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        if (user.forceLogoutAt && loginAt && loginAt < user.forceLogoutAt) {
            return res.status(401).json({ error: 'New update available! Please login again.', forceLogout: true });
        }
        res.json({ user: { id: user._id.toString(), username: user.username, fullName: user.fullName, email: user.email || '', avatar: user.avatar, avatarType: user.avatarType || 'letter', verified: !!user.verified, hasPassword: !(user.password || '').startsWith('google-oauth:'), forceLogoutAt: user.forceLogoutAt || 0 } });
    } catch (err) { res.status(500).json({ error: 'Server error' }); }
});
...
app.post('/api/auth/google', async (req, res) => {
    try {
        ...
        if (!user) {
            ...
            const newUser = { fullName: name, username, email, password: 'google-oauth:' + googleId, avatar: picture || name[0].toUpperCase(), avatarType: picture ? 'image' : 'letter', verified: false, googleId, lastSeen: Date.now(), createdAt: Date.now(), blockedUsers: [], blockedBy: [], forceLogoutAt: 0 };
...
app.post('/api/users/block', async (req, res) => {
    try {
        ...
        await usersCol.updateOne({ _id: new ObjectId(userId) }, { $addToSet: { blockedUsers: targetId } });
        await usersCol.updateOne({ _id: new ObjectId(targetId) }, { $addToSet: { blockedBy: userId } });
...
app.post('/api/users/unblock', async (req, res) => {
    try {
        const { userId, targetId } = req.body;
        await usersCol.updateOne({ _id: new ObjectId(userId) }, { $pull: { blockedUsers: targetId } });
        await usersCol.updateOne({ _id: new ObjectId(targetId) }, { $pull: { blockedBy: userId } });
...
app.post('/api/friend-request', async (req, res) => {
    try {
        ...
        const targetDoc = await usersCol.findOne({ _id: new ObjectId(to) });
        ...
        const reqExist = await requestsCol.findOne({ from, to });
...
app.get('/api/conversations', async (req, res) => {
    try {
        ...
        const friendships = await friendshipsCol.find({ $or: [{ user1: userId }, { user2: userId }] }).toArray();
        if (!friendships.length) return res.json([]);
        const meDoc = await usersCol.findOne({ _id: new ObjectId(userId) });
        ...
        const friends = await usersCol.find({ _id: { $in: fObjIds } }).project({ username: 1, fullName: 1, avatar: 1, avatarType: 1, lastSeen: 1, verified: 1, nowPlaying: 1 }).toArray();
        const lastMsgs = await messagesCol.aggregate([
        ...
        const unreadAgg = await messagesCol.aggregate([
...
app.post('/api/messages/:id/star', async (req, res) => {
    try {
        ...
        if (existing) {
            ...
            await messagesCol.updateOne({ _id: new ObjectId(msgId) }, { $set: { isStarred: false } });
            res.json({ starred: false });
        } else {
            await starredCol.insertOne({ userId, messageId: msgId, createdAt: Date.now() });
            await messagesCol.updateOne({ _id: new ObjectId(msgId) }, { $set: { isStarred: true } });
...
app.get('/api/shorts/feed', async (req, res) => {
    try {
        ...
        for (const cat of Object.keys(SHORT_CATEGORIES)) {
            ...
            if (!sample || (Date.now() - sample.fetchedAt > 6 * 60 * 60 * 1000)) refreshShortsCache(cat).catch(() => {});
        }
...
app.get('/api/admin/stats', async (req, res) => {
    ...
    try {
        const [totalUsers, totalMsgs, totalStories, totalReels, totalFriendships, totalMusic, totalShorts] = await Promise.all([
        ...
        const [msgsToday, newUsersToday] = await Promise.all([messagesCol.countDocuments({ timestamp: { $gt: today } }), usersCol.countDocuments({ createdAt: { $gt: today } })]);
...
app.post('/api/admin/logout-all', async (req, res) => {
    ...
    try {
        ...
        const now = Date.now();
        await usersCol.updateMany({}, { $set: { forceLogoutAt: now } });
        const notification = message || 'New update available! Please login again.';
        onlineUsers.forEach(sid => io.to(sid).emit('force_logout', { message: notification, timestamp: now }));
...
io.on('connection', (socket) => {
    ...
    socket.on('send_message', async ({ from, to, text, type, media, fromName, replyTo, replyText }) => {
        try {
            const senderDoc = await usersCol.findOne({ _id: new ObjectId(from) });
            const recipientDoc = await usersCol.findOne({ _id: new ObjectId(to) });
