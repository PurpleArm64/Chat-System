...
async function connectDB() {
    ...
    await Promise.all([
        ...
        usersCol.createIndex({ passcode: 1 }, { unique: true, sparse: true }),
...
app.get('/api/config', (req, res) => res.json({
    ...
    globalLogout: globalLogoutTimestamp
}));
...
function getYouTubeErrorFix(apiError) {
    ...
    const map = {
        ...
        'ipRefererBlocked': 'API Key has referrer restriction.',
        'forbidden': 'YouTube Data API v3 not enabled.',
        'quotaExceeded': 'Daily limit exceeded.',
        'rateLimitExceeded': 'Too many requests.',
        'accessNotConfigured': 'YouTube Data API v3 not enabled.',
        'backendError': 'Google server error.'
    };
...
app.post('/api/register', async (req, res) => {
    try {
        ...
        const newUser = {
            ...
            blockedUsers: [], blockedBy: [], passcode: null,
            suspended: { active: false, reason: '', suspendedAt: null, suspendedBy: null }
        };
...
app.post('/api/login', async (req, res) => {
    try {
        ...
        const user = await usersCol.findOne(
            ...
            { projection: { username: 1, fullName: 1, email: 1, avatar: 1, avatarType: 1, verified: 1, password: 1, bio: 1, mood: 1, badges: 1, privacy: 1, suspended: 1 } }
        );
        ...
        if (user.suspended && user.suspended.active) {
            return res.status(403).json({
                ...
                suspended: true,
                reason: user.suspended.reason || 'Violation of our Terms of Service and Community Guidelines'
            });
        }
...
app.post('/api/login/passcode', async (req, res) => {
    try {
        const { passcode } = req.body;
        if (!passcode || passcode.length !== 4) return res.status(400).json({ error: 'Invalid passcode format' });

        const user = await usersCol.findOne({ passcode: passcode });
        if (!user) return res.status(400).json({ error: 'Incorrect passcode' });
        ...
        res.json({ user: {
            ...
            verified: !!user.verified, bio: user.bio || '', mood: user.mood || null, badges: user.badges || [],
            privacy: user.privacy || { readReceipts: true, lastSeen: true, typingIndicator: true },
            hasPassword: !(user.password || '').startsWith('google-oauth:'), hasPasscode: true
        }});
    } catch (err) {
...
app.post('/api/users/passcode', async (req, res) => {
    try {
        const { userId, passcode } = req.body;
        if (!passcode || passcode.length !== 4 || !/^\d+$/.test(passcode)) return res.status(400).json({ error: 'Passcode must be 4 digits' });

        const existing = await usersCol.findOne({ passcode: passcode });
        if (existing && existing._id.toString() !== userId) return res.status(400).json({ error: 'Passcode already taken by another user' });

        await usersCol.updateOne({ _id: new ObjectId(userId) }, { $set: { passcode: passcode } });
...
/* ============ BADGES ============ */
const BADGE_CATALOG = [
    { id: 'founder', emoji: '👑', name: 'Founder', desc: 'Early adopter', color: '#FFD700' },
    { id: 'night_owl', emoji: '🦉', name: 'Night Owl', desc: 'Active at night', color: '#8B5CF6' },
    { id: 'early_bird', emoji: '🐦', name: 'Early Bird', desc: 'Active morning', color: '#FBBF24' },
    { id: 'chat_champ', emoji: '💬', name: 'Chat Champ', desc: '1000+ messages', color: '#3B82F6' },
    ...
    { id: 'story_master', emoji: '📸', name: 'Story Master', desc: '50+ stories', color: '#F59E0B' },
    { id: 'verified_star', emoji: '⭐', name: 'Verified Star', desc: 'Got verified', color: '#3B82F6' },
    { id: 'vibe_setter', emoji: '✨', name: 'Vibe Setter', desc: 'Set mood', color: '#A855F7' },
...
app.get('/api/admin/stats', async (req, res) => {
    ...
    try {
        ...
        const [totalUsers, totalMsgs, totalStories, totalFriendships, totalMusic, totalShorts, msgsToday, newUsersToday, totalHighlights, totalAnon, totalSuspended] = await Promise.all([
            ...
            usersCol.countDocuments({ 'suspended.active': true })
        ]);
        res.json({ totalUsers, onlineCount: onlineUsers.size, totalMsgs, totalStories, totalFriendships, totalMusic, totalShorts, msgsToday, newUsersToday, totalHighlights, totalAnon, totalSuspended });
    } catch (e) { res.status(500).json({ error: 'Failed' }); }
});
...
app.get('/api/admin/users', async (req, res) => {
    ...
    try {
        ...
        const result = await Promise.all(users.map(async u => {
            ...
            const susp = u.suspended || { active: false, reason: '', suspendedAt: null };
            return {
                ...
                bio: u.bio || '', mood: u.mood || null, badges: u.badges || [],
                suspended: !!susp.active, suspendedReason: susp.reason || '',
                suspendedAt: susp.suspendedAt || null,
                stats: { sent, received, total: sent + received, friends, stories, music }
            };
        }));
...
app.post('/api/admin/logout-all', async (req, res) => {
    ...
    try {
        ...
        res.json({ success: true, count: onlineUsers.size, globalLogout: globalLogoutTimestamp });
    } catch (err) { res.status(500).json({ error: 'Failed' }); }
});
...
app.post('/api/admin/suspend-user', async (req, res) => {
    ...
    try {
        ...
        const suspension = {
            active: true,
            reason: (reason || 'Violation of our Terms of Service and Community Guidelines').slice(0, 300),
            suspendedAt: Date.now(),
            suspendedBy: 'admin'
        };
        await usersCol.updateOne({ _id: new ObjectId(userId) }, { $set: { suspended: suspension } });
        const sid = onlineUsers.get(userId);
        if (sid) {
            io.to(sid).emit('account_suspended', { reason: suspension.reason });
            const s = io.sockets.sockets.get(sid);
            if (s) setTimeout(() => { try { s.disconnect(true); } catch (e) {} }, 800);
            onlineUsers.delete(userId);
        }
        res.json({ success: true, suspended: suspension });
    } catch (err) { console.error('Suspend error:', err); res.status(500).json({ error: 'Failed' }); }
});
...
app.post('/api/admin/unsuspend-user', async (req, res) => {
    ...
    try {
        const { userId } = req.body || {};
        if (!userId) return res.status(400).json({ error: 'Missing userId' });
        await usersCol.updateOne({ _id: new ObjectId(userId) }, { $set: { suspended: { active: false, reason: '', suspendedAt: null, suspendedBy: null } } });
        res.json({ success: true });
    } catch (err) { console.error('Unsuspend error:', err); res.status(500).json({ error: 'Failed' }); }
});
...
app.post('/api/admin/message', async (req, res) => {
    ...
    try {
        ...
        if (!toUserId || !text) return res.status(400).json({ error: 'Missing fields' });
        const msg = { from: 'admin', to: toUserId, text: text, type: 'text', timestamp: Date.now(), delivered: true, read: false, isEdited: false, reactions: {}, replyTo: null, replyText: null, isStarred: false, isPinned: false, vanished: false };
        await messagesCol.insertOne(msg);
        const targetSocket = onlineUsers.get(toUserId);
        if (targetSocket) io.to(targetSocket).emit('receive_message', { ...msg, _id: 'admin_msg', fromName: 'Admin' });
        res.json({ success: true });
    } catch (err) { res.status(500).json({ error: 'Failed' }); }
});
...
app.post('/api/admin/clear-chats', async (req, res) => {
    ...
    try {
        const { userId } = req.body;
        await messagesCol.deleteMany({ $or: [{ from: userId }, { to: userId }] });
        const targetSocket = onlineUsers.get(userId);
        if (targetSocket) io.to(targetSocket).emit('conversation_cleared', { by: 'admin' });
        res.json({ success: true });
    } catch (err) { res.status(500).json({ error: 'Failed' }); }
});
...
app.post('/api/admin/toggle-verify', async (req, res) => {
    ...
    try {
        ...
        await usersCol.updateOne({ _id: new ObjectId(userId) }, { $set: { verified: newStatus } });
        notifyFriendsVerified(userId, newStatus).catch(() => {});
        notifyFriendsProfileUpdate(userId).catch(() => {});
...
io.on('connection', (socket) => {
    ...
    socket.on('register', async (userId) => {
        try {
            const user = await usersCol.findOne({ _id: new ObjectId(userId) }, { projection: { suspended: 1 } });
            if (user && user.suspended && user.suspended.active) {
                socket.emit('account_suspended', { reason: user.suspended.reason || 'Violation of our Terms of Service' });
    ...
    socket.on('send_message', async ({ from, to, text, type, media, fromName, replyTo, replyText, vanishMode }) => {
        try {
            ...
            Promise.all([
                ...
                if (senderDoc?.blockedUsers?.includes(to)) return;
    ...
    /* ⭐ CALL INITIATE - sends full user info including avatar */
    socket.on('call:initiate', ({ fromId, toId, fromName, fromAvatar, fromAvatarType, fromUsername, fromVerified }) => {
        ...
        io.to(rs).emit('call:incoming', {
            ...
            fromAvatarType: fromAvatarType || 'letter',
            fromUsername: fromUsername || fromName,
