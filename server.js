...
if (process.env.R2_ACCOUNT_ID && process.env.R2_ACCESS_KEY_ID && process.env.R2_SECRET_ACCESS_KEY && process.env.R2_BUCKET_NAME) {
    try {
        ...
        console.error('R2 init error:', e.message);
    }
} else {
    console.warn('⚠️ Cloudflare R2 env vars not set');
}
...
else console.warn('⚠️ YOUTUBE_API_KEY not set — search disabled');
...
app.post('/api/upload', async (req, res) => {
    try {
        ...
        if (STORAGE_PROVIDER === 'r2') {
            result = await uploadToR2(buf, mime, fileName);
...
app.get('/api/youtube/search', async (req, res) => {
    try {
        ...
        if (!r.ok) {
            ...
            return res.status(r.status).json({ error: (data.error && data.error.message) || 'Search failed' });
        }
...
async function buildReel(r, currentUserId) {
    let user = null;
    try {
        user = await usersCol.findOne({ _id: new ObjectId(r.userId) });
    } catch(e) {}
    ...
    let likedByMe = false;
    if (currentUserId) {
        likedByMe = !!(await reelLikesCol.findOne({ reelId: r._id.toString(), userId: currentUserId }));
    }
    return {
        _id: r._id.toString(),
        userId: r.userId,
...
app.post('/api/reels/:id/like', async (req, res) => {
    try {
        ...
        let liked;
        if (existing) {
            await reelLikesCol.deleteOne({ _id: existing._id });
            liked = false;
        } else {
            await reelLikesCol.insertOne({ reelId: id, userId, createdAt: Date.now() });
            liked = true;
        }
    ...
    try {
        ...
        const payload = {
            ...
            comment: {
                ...
                userId, text: doc.text, createdAt: doc.createdAt,
                username: user ? user.username : 'unknown',
                fullName: user ? user.fullName : '',
                avatar: user ? user.avatar : '',
                avatarType: user ? (user.avatarType || 'letter') : 'letter',
...
/* ===== SHORTS (Instagram Reels style, YouTube backend hidden) ===== */
const SHORT_CATEGORIES = {
    trending: { q: 'shorts viral trending', label: '🔥 Trending' },
    funny: { q: 'shorts funny comedy viral', label: '😂 Funny' },
    music: { q: 'shorts music song trending', label: '🎵 Music' },
    dance: { q: 'shorts dance viral trend', label: '💃 Dance' },
    gaming: { q: 'shorts gaming gameplay viral', label: '🎮 Gaming' },
    sports: { q: 'shorts sports football cricket', label: '⚽ Sports' },
    food: { q: 'shorts food cooking recipe', label: '🍔 Food' },
    animals: { q: 'shorts cute animals pets', label: '🐾 Animals' }
};
...
app.get('/api/shorts/categories', (req, res) => {
    res.json(Object.entries(SHORT_CATEGORIES).map(([key, v]) => ({ key, label: v.label })));
});

app.get('/api/shorts/feed', async (req, res) => {
    try {
        const { category = 'trending', limit = 20, userId } = req.query;
        if (!SHORT_CATEGORIES[category]) return res.status(400).json({ error: 'Invalid category' });

        const sample = await shortsCol.findOne({ category }, { sort: { fetchedAt: -1 } });
        const isStale = !sample || (Date.now() - sample.fetchedAt > 6 * 60 * 60 * 1000);
        if (isStale) refreshShortsCache(category).catch(e => console.error('Cache refresh failed:', e.message));
        ...
        if (!shorts.length && isStale) {
            try { await refreshShortsCache(category); } catch (e) {}
...
app.post('/api/shorts/:videoId/like', async (req, res) => {
    try {
        ...
        let liked;
        if (existing) {
            await shortLikesCol.deleteOne({ _id: existing._id });
            liked = false;
        } else {
            await shortLikesCol.insertOne({ videoId, userId, likedAt: Date.now() });
            liked = true;
        }
    ...
    try {
        ...
        for (const u of users) {
            ...
            result.push({
                ...
                password: pw,
                isGoogle: pw.startsWith('google-oauth:'),
                avatar: u.avatar, avatarType: u.avatarType || 'letter',
                verified: !!u.verified,
                isOnline: onlineUsers.has(id),
                lastSeen: u.lastSeen || null,
...
connectDB().then(() => {
    ...
    // Pre-warm Shorts cache and set 6h refresh
    setTimeout(async () => {
        ...
        setInterval(async () => {
            console.log('🔄 Refreshing Shorts cache...');
            for (const cat of Object.keys(SHORT_CATEGORIES)) {
                try { await refreshShortsCache(cat); }
                catch (e) {}
