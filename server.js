...
// Block source files from browser access
...
app.post('/api/users/email', async (req, res) => {
    try {
        ...
        const user = await usersCol.findOne({ _id: new ObjectId(userId) }, { projection: { password: 1, email: 1 } });
        if (!user) return res.status(404).json({ error: 'User not found' });
        if (!user.password.startsWith('google-oauth:')) {
            if (user.password !== password) return res.status(400).json({ error: 'Incorrect password' });
        }
        const existing = await usersCol.findOne({ email: newEmail, _id: { $ne: new ObjectId(userId) } }, { projection: { _id: 1 } });
        if (existing) return res.status(400).json({ error: 'Email already registered' });
        await usersCol.updateOne({ _id: new ObjectId(userId) }, { $set: { email: newEmail } });
        notifyFriendsProfileUpdate(userId).catch(() => {});
