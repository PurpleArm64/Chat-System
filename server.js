    ...
    try {
        ...
        if (STORAGE_PROVIDER === 'r2') result = await uploadToR2(buf, mime, fileName);
            ...
            try {
                ...
                console.warn('R2 failed, trying Cloudinary:', e.message);
...
app.post('/api/tiktok/resolve', async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ error: 'Failed to resolve TikTok URL' }); }
});
...
app.post('/api/youtube/oembed', async (req, res) => {
    ...
    } catch (err) { res.status(500).json({ error: 'Failed to fetch video info' }); }
});
    ...
    try {
        ...
        if (avatar && avatar.startsWith('data:')) {
            ...
            if (match) {
                ...
                try {
                    let result = null;
                    if (r2Ready) result = await uploadToR2(buf, mime, 'avatar_' + Date.now());
                    else if (cloudinaryReady) result = await uploadToCloudinary(buf, mime);
                    if (result) { avatarUrl = result.url; avatarType = mime === 'image/gif' ? 'gif' : 'image'; }
...
app.post('/api/users/avatar', async (req, res) => {
    try {
        ...
        if (avatar.startsWith('data:')) {
            ...
            if (match) {
                const mime = match[1];
                const buf = Buffer.from(match[2], 'base64');
                if (buf.length > 3 * 1024 * 1024) return res.status(400).json({ error: 'File too large (max 3MB)' });
                let result = null;
                try {
                    if (r2Ready) result = await uploadToR2(buf, mime, 'avatar_' + Date.now());
                    else if (cloudinaryReady) result = await uploadToCloudinary(buf, mime);
                ...
                if (result) {
                    ...
                    avatarType = mime === 'image/gif' ? 'gif' : 'image';
                }
            }
        }
    ...
    } catch (err) { console.error('Avatar error:', err); res.status(500).json({ error: 'Server error' }); }
});
...
app.post('/api/users/block', async (req, res) => {
    try {
        ...
        const selfSid = onlineUsers.get(userId);
        if (selfSid) io.to(selfSid).emit('user_blocked', { userId, targetId });
        const targetSid = onlineUsers.get(targetId);
        if (targetSid) io.to(targetSid).emit('you_were_blocked', { by: userId });
        res.json({ success: true });
...
app.post('/api/users/unblock', async (req, res) => {
    try {
        ...
        const selfSid = onlineUsers.get(userId);
        if (selfSid) io.to(selfSid).emit('user_unblocked', { userId, targetId });
        res.json({ success: true });
...
app.get('/api/users/blocked', async (req, res) => {
    try {
        ...
        const user = await usersCol.findOne({ _id: new ObjectId(userId) });
        if (!user) return res.status(404).json({ error: 'User not found' });
        const blockedIds = (user.blockedUsers || []).map(id => { try { return new ObjectId(id); } catch (e) { return null; } }).filter(Boolean);
        const blocked = await usersCol.find({ _id: { $in: blockedIds } })
            .project({ username: 1, fullName: 1, avatar: 1, avatarType: 1, verified: 1 }).toArray();
        res.json(blocked.map(u => ({
            id: u._id.toString(), username: u.username, fullName: u.fullName,
            avatar: u.avatar, avatarType: u.avatarType || 'letter', verified: !!u.verified
        })));
    } catch (err) { res.status(500).json({ error: 'Server error' }); }
});
...
app.post('/api/friend-request', async (req, res) => {
    try {
        ...
        if (targetDoc?.blockedUsers?.includes(from)) return res.status(400).json({ error: 'Cannot send request to this user' });
...
app.get('/api/conversations', async (req, res) => {
    try {
        ...
        const blockedByMe = meDoc?.blockedUsers || [];
        const blockedMe = meDoc?.blockedBy || [];
        const friendIds = friendships
            .map(f => f.user1 === userId ? f.user2 : f.user1)
            .filter(fid => !blockedByMe.includes(fid) && !blockedMe.includes(fid));
        const results = [];
...
const SHORT_CATEGORIES = {
    trending:    { q: 'shorts Instagram trend' },
...
app.get('/api/shorts/feed', async (req, res) => {
    try {
        ...
        res.json(shorts.map(s => ({
            videoId: s.videoId, title: s.title, thumbnail: s.thumbnail,
            author: s.author, category: s.category,
            liked: likedIds.has(s.videoId),
            likesCount: countMap.get(s.videoId) || 0
        })));
    } catch (e) { console.error('Shorts feed error:', e); res.status(500).json({ error: e.message }); }
});
...
io.on('connection', (socket) => {
    ...
    socket.on('send_message', async ({ from, to, text, type, media, fromName }) => {
        try {
            ...
            if (recipientDoc?.blockedUsers?.includes(from)) return socket.emit('message_error', { error: 'You are blocked by this user' });
